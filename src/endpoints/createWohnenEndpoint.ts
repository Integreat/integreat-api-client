...
type ParamsType = { city: string | null | undefined; };

export default (baseUrl: string): Endpoint<ParamsType, Array<WohnenOfferModel>> => new EndpointBuilder<ParamsType, Array<WohnenOfferModel>>(WOHNEN_ENDPOINT_NAME).withParamsToUrlMapper((params): string => {
  if (!params.city) {
    throw new ParamMissingError(WOHNEN_ENDPOINT_NAME, 'city')
  }

  return `${baseUrl}/${params.city}/offer`
}).withMapper((json: Array<OfferType>): Array<WohnenOfferModel> => json.map(offer => {
...
}).sort((model1, model2) => model1.createdDate.isBefore(model2.createdDate) ? 1 : -1)).build()
