...
describe('events', () => {
  ...
  const createEvent = (id: number, allDay: boolean, startDate: string, startTime: string, endDate: string, endTime: string, timezone: string): JsonEventType => ({
    id,
  ...
  const createEventModel = (id: number, allDay: boolean, startDate: Moment, endDate: Moment): EventModel => new EventModel({
    path: '/augsburg/de/events/asylpolitischer_fruehschoppen',
    title: 'Asylpolitischer Frühschoppen',
  ...
  const eventModel4 = createEventModel(4826, true, moment.tz('2018-03-01 00:00:00', 'America/New_York'), moment.tz('2018-06-01 23:59:59', 'America/New_York'))

  const params = { city: 'augsburg', language: 'de' }

  it('should map params to url', () => {
    expect(events.mapParamsToUrl(params)).toEqual('https://integreat-api-url.de/augsburg/de/wp-json/extensions/v3/events')
  })
